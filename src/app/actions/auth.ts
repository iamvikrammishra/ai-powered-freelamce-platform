...
export async function createUserProfile(
  ...
  try {
    ...
    // IMPORTANT: Add a short delay to ensure auth user is fully committed in the database
    ...
    if (userError || !authUser.user) {
      console.error('User does not exist in auth.users yet:', userError || 'No user found');
      return { 
        success: false, 
        error: 'User account not fully created yet. Please try again or contact support.'
      };
    }
