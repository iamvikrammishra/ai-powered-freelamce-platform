...
export default function LoginPage() {
  ...
  const handleEmailLogin = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // Get user details including role and name
      if (data.user) {
        // Get user metadata
        const userRole = data.user.user_metadata?.role as string || 'freelancer';
        const fullName = data.user.user_metadata?.full_name as string || '';
        
        // Set user info for welcome modal
        ...
        // Fallback to direct redirect if no user data
  ...
  const handleSendOTP = async () => {
    ...
    try {
      ...
      toast({
        title: "OTP sent",
        description: `We've sent an OTP to ${phone}`,
      })
    } catch (error: any) {
      toast({
  ...
  const handleVerifyOTP = async (e: React.FormEvent) => {
    ...
    try {
      ...
      toast({
        title: "Login successful",
        description: "Welcome back to GigIndia!",
      })
  ...
  // Handle welcome modal close - directly navigate to dashboard
  const handleCloseWelcomeModal = () => {
    // First hide the modal
    setShowWelcomeModal(false)
    
    // Then navigate to dashboard - using replace to avoid navigation history issues
  ...
  // This is crucial for handling successful authentication
  useEffect(() => {
    // Set up auth state listener to detect login and redirect
    const { data: { subscription } } = supabase.auth.onAuthStateChange((event, session) => {
      if (event === 'SIGNED_IN' && session) {
        // Get user metadata
        const userRole = session.user?.user_metadata?.role as 'freelancer' | 'employer' || 'freelancer';
      ...
      <Card className="w-full max-w-md">
        ...
        <CardFooter className="flex justify-center">
          <p className="text-sm text-gray-600 dark:text-gray-400">
            ...
            <Link href="/auth/register" className="text-purple-600 hover:text-purple-700 dark:text-purple-400">
              Sign up
            </Link>
          </p>
        </CardFooter>
